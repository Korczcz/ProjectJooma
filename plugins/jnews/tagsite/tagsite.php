	...
	preg_match_all('#\{tag:viewonline.{3,}?\}#', $html, $tags);
 	...
 	if ( !empty($tags[0]) ) {
 		...
 		foreach( $tags[0] as $tag ) {
			...
			$replace[$tag] = $tag;
			$Itemid = jNewsConfig::get( 'itemidAca' );
			$mailingId = $mainframe->input->getInt( 'mailingid', 0 );
 			if($mailingId == 0)
			{	
					if(isset($queueInfo->id))$mailingId = $queueInfo->id;				
			}

			$listId = $mainframe->input->getInt( 'listid', 0 );

			//create the link
			$link = jNews_Tools::completeLink( 'option='.JNEWS_OPTION.'&act=mailing&task=view&mailingid='.$mailingId . $themURL , false, jNewsConfig::get( 'use_sef' ) );
			...
			$mainframe = JFactory::getApplication();
$act = $mainframe->input->get('act', '', '', 'WORD' );
			$mainframe = JFactory::getApplication();
			if ( !$mainframe->isClient('administrator') && $task=='view' && $act=='mailing' ) {
				$replacebyHTML[$tag] = '';
				$replacebyText[$tag] = '';
			} else {
